...
abstract class TypedSponsorflowAgent<out P : AgentPayload, out R : AgentResponseData> : SponsorflowAgent() {
    ...
    protected abstract suspend fun executeTypedInternal(task: SwarmTask<@UnsafeVariance P>): SwarmResult<R, SwarmError>

    suspend fun executeTypedTask(task: SwarmTask<@UnsafeVariance P>): SwarmResult<R, SwarmError> {
        return try {
